...
5. The draft [`v0.5 execution package`](../04-execution/deprail-v0.5.0-execution-package/README.md), requirements, and owner-reviewed issue contracts. The package remains in draft until its technical design gates and owner Definition of Ready pass.
...
v0.4.0-preview.2 was published with an owner-approved `go with approved gaps`, but issue #356 remains open. Its former independent architecture/security review follow-up is superseded and owner-dispositioned by [ADR-0005](../adr/ADR-0005-solo-owner-review-policy.md); the owner security/architecture assessment remains part of current release evidence. The four remaining follow-ups—rollback ownership/recovery procedure, Python/Java remediation coverage, cross-platform representative smoke, and signature/provenance dispositions—remain open in #356. Preserve the fifth follow-up as historical tracking, not as a current external-review gate. Before Definition of Ready approval, #387 must link this per-item disposition and ensure incomplete ownership/targets are resolved in #356.
...
| Local web console | Product design §§Web Product Design, Technical Architecture; architecture §§4, 13, 16 | v0.5 local web and history | Include an embedded React/TypeScript/Vite console using the existing application layer. User preference: shadcn/ui-inspired style, subject to owner acceptance of the design-system and accessibility decisions. |
| Local REST API | Architecture §§4, 5, 7.4, 13 | v0.5 local web and history | Include only the API surface required by the accepted local workflows; define and owner-accept the OpenAPI contract and local security boundary before implementation. |
...
- An embedded React, TypeScript, and Vite console built as static assets and served by the local application. Use a shadcn/ui-inspired visual direction: accessible, composable primitives, consistent spacing/type/color tokens, and clear state surfaces. The owner decides the library, dependencies, and component approach.
...
- Changes to existing CLI/JSON contracts unless an owner-approved compatibility decision requires them.
...
The following sequence is mandatory. UX and API design are completed, technically evidenced, and accepted by the owner before any runtime implementation issue is created or started. Their work may proceed in parallel only after product scope and shared domain/resource assumptions have been reconciled.

| Order | Stage | Required deliverable | Gate / dependency |
| ---: | --- | --- | --- |
| 0 | Release reconciliation | Owner-accepted scope, v0.4 follow-up dispositions, owner reviewer, compatibility and risk decisions | Complete #387 planning work; no runtime authorization. |
| 1 | Product workflows and UX research | Target users/jobs, primary navigation and routes, workflow/state diagrams, information architecture, wireframes/prototype, visual tokens, accessibility checklist, error/empty/loading/partial/failed states | Owner accepts UX package; shadcn/ui direction and dependency strategy recorded. External design review is optional. |
| 2 | API and data contract design | Versioned OpenAPI 3.1 proposal, resource and error models, endpoint/method matrix, pagination/filtering semantics if needed, request limits, bind/origin/security rules, cancellation semantics, compatibility/versioning policy | Owner accepts the contract and required security evidence; no endpoint or schema is assumed beforehand. |
| 3 | Persistence architecture decision | SQLite schema and migration plan, transactions/concurrency, artifact references, retention/deletion, corruption/disk-full behavior, backup/recovery, file permissions | Owner accepts ADR and data contract; crosswalk to existing scan/artifact contracts. |
| 4 | Execution package and implementation decomposition | Requirements, threat model, test strategy, compatibility matrix, ordered epics/issues, acceptance evidence, rollback and release plan | UX, API, and persistence design gates pass; owner accepts the Definition of Ready. |
| 5 | Implementation | Ordered issue work derived from accepted contracts (persistence/service first; API transport; UI integration; packaging/docs) | Only then create implementation issues and start code; one bounded issue at a time. |
| 6 | Release verification | Real local workflows, storage/API/UI failure evidence, cross-platform packaging/smoke, owner security assessment, artifacts, owner release decision | Version-specific evidence complete or gaps explicitly accepted by the owner for preview. |
...
Before API implementation, the owner-reviewed OpenAPI contract and security assessment must define:
...
The gate requires owner approval. External review is optional under ADR-0005. At minimum:

- Owner-reviewed UX package, API/OpenAPI contract, persistence decision, threat model, and execution package are linked from implementation contracts before runtime work.
...
- UI matches owner-accepted flows; keyboard/screen-reader/focus/contrast/responsive/reduced-motion states are verified on the actual surface.
...
- The owner’s security/architecture assessment and release decision are recorded as distinct evidence; preview gaps, rollback owner/procedure, artifact checksums/SBOM/signature/provenance status, and retrospective follow-ups are explicit.
...
v0.5 consumes roadmap capacity planned for the local web/history milestone. Team collaboration and PostgreSQL remain displaced to v0.6; scoped MCP/agent work remains v0.7; Trivy/external adapters remain v0.8. No additional capability is displaced or added without a documented owner decision. If owner assessment shows that the local web outcome cannot meet the security or compatibility boundary, pause implementation and record a versioned scope decision rather than silently weakening the gate.
...
- [ ] The four open #356 technical follow-ups have explicit owner/target dispositions; the former independent-review item is dispositioned as superseded by ADR-0005 and is not a blocker.
- [ ] Owner confirms included/excluded capabilities, compatibility boundary, failure model, preview mode, and acceptance gate.
- [ ] Owner reviews and records the decision on the exact current contract versions; independent review is optional under ADR-0005.
...
- [ ] SQLite/data lifecycle ADR and migration/failure contracts are owner-reviewed and accepted.
...
- [ ] Every proposed implementation issue maps to an accepted contract, has one primary outcome, explicit dependencies, observable acceptance, failure behavior, owner reviewer, and evidence.
