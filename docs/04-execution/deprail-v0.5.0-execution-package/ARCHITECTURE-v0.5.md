...
| Domain | Deterministic scan identity, report meaning, completeness, provenance | React, transport-specific types, SQL |
| History port | Define operations and stable data contract for storing/retrieving accepted history | Rendering or transport details |
...
| API transport | Validate request shape/size, map application outcomes/errors, enforce listener/origin rules | SQL queries or filesystem traversal |
...
4. History adapter returns versioned report metadata and stable artifact references; the application verifies applicable integrity/provenance contracts.
5. Transport returns a bounded response whose semantics distinguish complete, partial, failed, cancelled, and unavailable states.
6. The browser renders those states without deriving domain meaning from colors or assuming missing data means empty data.
...
- Complete/partial/failed/cancelled meanings survive storage, API, UI, and platform conversion.
- Stable scan and finding identifiers/provenance remain governed by current domain contracts.
...
Storage unavailable, migration rejected, database corrupt/locked/full, artifact missing or digest mismatch, malformed/oversized request, timeout, cancellation, unsupported contract version, listener failure, and missing embedded UI assets are typed explicit failures. None becomes an empty successful scan/history response. A failure to save history must not rewrite the scan's own completeness state; the user sees both the scan outcome and persistence outcome distinctly.
