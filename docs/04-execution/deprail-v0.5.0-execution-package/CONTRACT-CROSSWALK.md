...
3. Independently review the candidate [`OpenAPI 3.1 artifact`](../../../schemas/openapi/v1/openapi.yaml), merged as a candidate in [PR #413](https://github.com/geoffrey-xiao/deprail/pull/413) and tightened in follow-up [PR #414](https://github.com/geoffrey-xiao/deprail/pull/414); the merge is not independent acceptance. A direct review request is recorded in #414 because GitHub rejected formal assignment (HTTP 422; reviewer is not a repository collaborator). Review exact projection fields, pagination/cursors, error/status mapping, version policy, and numeric request/response limits. Offline checks are evidence, not acceptance.
