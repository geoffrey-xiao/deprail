...
3. Independently review the current [`OpenAPI 3.1 artifact`](../../../schemas/openapi/v1/openapi.yaml), corrected in [PR #414](https://github.com/geoffrey-xiao/deprail/pull/414). The owner merged that PR, but its only submitted review is Codex COMMENTED; no independent approval is recorded. Its negative phrase `close #396` inadvertently triggered GitHub auto-close; #396 has been reopened and returned to Project Review. The direct reviewer request remains in #414; formal assignment failed with HTTP 422 because the reviewer is not a repository collaborator. Offline checks are evidence, not acceptance.
...
The [V05-005 readiness map](issues/V05-005-readiness-map.md#conditional-post-dor-delivery-map) groups FR-501–FR-511 and SEC/STORE evidence into tentative store, shared-service, API, UI, packaging, and release-verification outcomes. It is not an implementation backlog: #396 is reopened in Project Review, #393 remains in Review, ADR-0004 remains Proposed, and the current PR #414 API candidate lacks independent review. Final issue ownership, labels, interfaces, dependencies and acceptance evidence follow separately approved contracts, not this provisional grouping.
