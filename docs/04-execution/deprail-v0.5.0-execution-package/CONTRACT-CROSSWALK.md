...
| Architecture §§4, 5, 7.3–7.5 | Shared app services; local REST; embedded React; SQLite | [`ARCHITECTURE-v0.5.md`](ARCHITECTURE-v0.5.md), [`API-DESIGN.md`](API-DESIGN.md), failure/data requirements | Owner-accepted contracts, OpenAPI and SQLite ADR before code |
...
| v0.5 development plan §6 | UX and API design gates precede implementation | [`UX-DESIGN.md`](UX-DESIGN.md), [`API-DESIGN.md`](API-DESIGN.md), README DoR | Owner approval before implementation issue creation; external review optional |
...
| v0.4 requirements | No `requirements/ERROR-MODEL.md` exists in the v0.4 package; its available requirements do not replace v0.1/v0.2 scan codes | v0.5 [`requirements/ERROR-MODEL.md`](requirements/ERROR-MODEL.md) | Name and validate the actual contract instead of citing a nonexistent v0.4 error model |
...
## Remaining owner decisions and evidence gates
...
**Owner-selected direction, exact contract not accepted:** [ADR-0004's additive finding and direction](../../adr/ADR-0004-local-scan-history.md#owner-selected-direction-independent-history-projection-not-accepted) keeps existing `ScanReport`/CLI serialization unchanged and proposes a separate allowlisted `history-v1` projection. The candidate storage layout, read projection, error mapping, and evidence matrix reflect that direction; exact field and unknown-data rules still need owner acceptance before API schema freeze or implementation.

The owner recorded approval of the then-current package and the independent reviewer explicitly approved [PR #412](https://github.com/geoffrey-xiao/deprail/pull/412). Those historical decisions predate this OpenAPI artifact and local security profile. The owner merged the corrections in [PR #414](https://github.com/geoffrey-xiao/deprail/pull/414); no independent APPROVED review is recorded, and none is required under [ADR-0005](../../adr/ADR-0005-solo-owner-review-policy.md). Runtime work and implementation issues remain gated by the current technical decisions and complete owner Definition of Ready.

1. The owner accepts or revises exact `history-v1` field mapping/validation, safe diagnostics, unknown-data policy, and the candidate history-capture trigger/CLI compatibility behavior, including projection/persistence-failure and exit-code precedence.
...
3. The owner reviews and records an explicit decision on the current [`OpenAPI 3.1 artifact`](../../../schemas/openapi/v1/openapi.yaml), corrected in [PR #414](https://github.com/geoffrey-xiao/deprail/pull/414). Its negative phrase `close #396` inadvertently triggered GitHub auto-close; #396 has been reopened and returned to Project Review. External review is optional under ADR-0005; offline checks remain evidence, not owner acceptance.
4. The owner accepts or revises local listener bind/port/lifetime, Host/Origin/CORS/CSRF/authentication/token transfer, and shutdown behavior; explicitly disposition transient browser/OS token-fragment exposure.
5. Select the shadcn/ui-compatible component/dependency strategy and freeze supported browser/OS/accessibility combinations through the owner's decision.

No implementation issue map is authorized until the remaining decisions, complete technical Definition of Ready, and owner approval for the current contract version have linked evidence. External review is optional. Refer to the release plan for the design-first sequence.


## Conditional delivery trace

The [V05-005 readiness map](issues/V05-005-readiness-map.md#conditional-post-dor-delivery-map) groups FR-501–FR-511 and SEC/STORE evidence into tentative store, shared-service, API, UI, packaging, and release-verification outcomes. It is not an implementation backlog: #396 is reopened in Project Review, #393 remains in Review, ADR-0004 remains Proposed, and the current technical API decisions remain open. External review is optional under ADR-0005. Final issue ownership, labels, interfaces, dependencies and acceptance evidence follow owner-accepted contracts, not this provisional grouping.
