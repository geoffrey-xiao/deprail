...
| API/OpenAPI and local listener | [PR #404](https://github.com/geoffrey-xiao/deprail/pull/404) merged a candidate profile; [#396](https://github.com/geoffrey-xiao/deprail/issues/396) remains in Review. [API-DESIGN.md](API-DESIGN.md) has no accepted OpenAPI artifact. | Freeze and validate OpenAPI 3.1 resources/examples, pagination/cursors, version/error mapping, numeric bounds and listener/Host/Origin/CORS/CSRF/auth/shutdown rules with independent architecture/security review. |
| SQLite and data lifecycle | [#391](https://github.com/geoffrey-xiao/deprail/issues/391) closed after owner-approved [PR #407](https://github.com/geoffrey-xiao/deprail/pull/407); [ADR-0004](../../adr/ADR-0004-local-scan-history.md) remains Proposed. | Resolve payload/store bounds, retention-growth risk, approved opt-in capture and persistence-failure precedence; record independent review of permissions, migrations, backup/recovery and artifacts. |
...
**Additional DoR blocker:** The actual scan report is neither privacy-safe unchanged JSON nor v1alpha finding/error-schema-valid. [ADR-0004 records the validation finding](../../adr/ADR-0004-local-scan-history.md#post-proposal-validation-finding-source-report-is-not-the-proposed-stored-document); [#396](https://github.com/geoffrey-xiao/deprail/issues/396) and the history-data contract must reconcile a versioned safe projection or an explicitly approved compatibility change before OpenAPI/storage acceptance. Owner and independent reviewer must decide; no raw-report persistence is authorized.
