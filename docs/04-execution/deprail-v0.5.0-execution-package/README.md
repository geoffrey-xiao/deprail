...
| API/OpenAPI and local listener | [PR #404](https://github.com/geoffrey-xiao/deprail/pull/404) merged a candidate profile; [#396](https://github.com/geoffrey-xiao/deprail/issues/396) remains in Review. The owner-selected, separately versioned history-projection direction is recorded in [merged PR #411](https://github.com/geoffrey-xiao/deprail/pull/411). The current [OpenAPI 3.1 candidate](../../../schemas/openapi/v1/openapi.yaml) and local security profile are submitted for independent review in [PR #413](https://github.com/geoffrey-xiao/deprail/pull/413), with a direct reviewer mention. GitHub rejected formal assignment with HTTP 422 because the named reviewer is not a repository collaborator; no permission was changed. | Reviewer decision on exact projection/API/security details and separate owner acceptance remain required; the candidate passed offline checks but is not accepted or runtime-tested. |
...
| Independent review and DoR sign-off | The owner recorded approval for the then-current package in [#392](https://github.com/geoffrey-xiao/deprail/issues/392#issuecomment-5807906998), and [`@geoffreyxiaoai`](https://github.com/geoffreyxiaoai) approved the full-package review in [PR #412](https://github.com/geoffrey-xiao/deprail/pull/412). That review predates the new OpenAPI artifact and concrete security profile. | A direct independent-review request is recorded in [PR #413](https://github.com/geoffrey-xiao/deprail/pull/413) and #396; GitHub will not formally assign the reviewer because the account is not a repository collaborator. Record the current-contract decision and owner acceptance before closing #396 or authorizing runtime work. |
...
**Current API design blocker:** The actual scan report is neither privacy-safe unchanged JSON nor v1alpha finding/error-schema-valid. [ADR-0004 records the validation finding and owner-selected separate projection](../../adr/ADR-0004-local-scan-history.md#owner-selected-direction-independent-history-projection-not-accepted). The OpenAPI candidate now provides a strict API shape, but exact `history-v1` fields, unknown/safe-diagnostic handling, SQL validation, row/API mapping and source-version compatibility still need current independent review and owner acceptance; no raw-report persistence, public API freeze, or runtime work is authorized.

## No implementation authorization

The owner and independent reviewer approved the then-current v0.5 package, but the OpenAPI/security candidate added afterward remains under #396 review. No SQL schema, public API, UI dependency, endpoint, serving behavior, or migration is final. No issue may authorize runtime implementation until all current package gates have linked evidence and separate owner/reviewer decisions.
