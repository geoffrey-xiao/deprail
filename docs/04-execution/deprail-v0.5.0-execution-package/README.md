...
These defaults consolidate current proposals for owner review; they are not accepted contracts or implementation authorization. Before marking a DoR item complete, record explicit owner decisions and link evidence appropriate to the planning stage. Actual runtime/browser/platform verification remains an implementation/release acceptance gate and is not claimed here.
...
| UX and platform support | Retain the existing React/TypeScript/Vite baseline; add no component, icon, chart, font, CDN, or analytics dependency. Use project-owned semantic components and local assets; target WCAG 2.2 AA. Start with one explicit browser/assistive-technology pair per required OS: Chrome/NVDA on Windows, Safari/VoiceOver on macOS, Firefox/Orca on Linux. | Before implementation, the owner selects the matrix and records its test plan. Verify the actual surface—keyboard/focus, screen-reader status, contrast, 320 CSS px/200% zoom, reduced motion, hostile repository text, and browser Back/Forward—during implementation/release acceptance on each declared combination. Record versions and unsupported combinations. |
