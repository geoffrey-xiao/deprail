...
| API/OpenAPI and local listener | [PR #404](https://github.com/geoffrey-xiao/deprail/pull/404) merged a candidate profile; the owner-selected `history-v1` direction is in [PR #411](https://github.com/geoffrey-xiao/deprail/pull/411). [PR #413](https://github.com/geoffrey-xiao/deprail/pull/413) merged the OpenAPI/security candidate, but does not close #396; five automated-review gaps are addressed in follow-up [PR #414](https://github.com/geoffrey-xiao/deprail/pull/414). #396 remains open in Review. | Independent human review of PR #414 and separate owner acceptance of the exact current API/security decisions remain required; the contract is not runtime-tested. Formal assignment failed with HTTP 422 because the named reviewer is not a repository collaborator. |
...
| Independent review and DoR sign-off | The owner approved the then-current package in [#392](https://github.com/geoffrey-xiao/deprail/issues/392#issuecomment-5807906998), and [`@geoffreyxiaoai`](https://github.com/geoffreyxiaoai) reviewed it in [PR #412](https://github.com/geoffrey-xiao/deprail/pull/412); that review predates the OpenAPI artifact and concrete security profile. A direct review request for the current corrections is in [PR #414](https://github.com/geoffrey-xiao/deprail/pull/414); formal assignment failed with HTTP 422 because the reviewer is not a repository collaborator. | Record independent review of the exact current contract and separate owner acceptance; keep #396 open and implementation blocked. |
...
**Current API design blocker:** The actual scan report is neither privacy-safe unchanged JSON nor v1alpha finding/error-schema-valid. [ADR-0004 records the validation finding and owner-selected separate projection](../../adr/ADR-0004-local-scan-history.md#owner-selected-direction-independent-history-projection-not-accepted). The OpenAPI candidate merged in [PR #413](https://github.com/geoffrey-xiao/deprail/pull/413); five automated-review schema gaps are corrected in follow-up [PR #414](https://github.com/geoffrey-xiao/deprail/pull/414). Exact `history-v1` fields, safe-diagnostic handling, SQL validation, row/API mapping, source-version compatibility, independent review, and owner acceptance remain pending; no runtime work is authorized.

## No implementation authorization

The owner and independent reviewer approved the then-current v0.5 package; the later OpenAPI/security candidate merged in PR #413, with schema corrections under review in PR #414. No SQL schema, public API, UI dependency, endpoint, serving behavior, or migration is final. No issue may authorize runtime implementation until all current package gates have linked evidence and separate owner/reviewer decisions.
