...
| SEC-05 | Oversized body/result or request flood | Candidate finite request-target/header/response/page/concurrency/deadline bounds, bodyless routes, and bounded shutdown; over-limit targets/headers are rejected at the parser boundary as 414/431 before routing and may not carry an API JSON envelope. Test boundaries and concurrency without truncation or resource exhaustion; exact numeric limits remain unapproved. |
