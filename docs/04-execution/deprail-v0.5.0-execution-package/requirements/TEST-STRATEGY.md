...
| --- | --- | --- | --- |
| FR-501 | Save/read two operation entries with the same source `ScanReport.ScanID`; require distinct history IDs and unchanged report identity/provenance. Retry the same operation using its ID; it must not create a duplicate. | Storage integration output, fixture and DB digests, round-trip values. | SQLite behavior on Linux, macOS, Windows. |
...
| FR-503 | Load detail with report, without report, missing artifact, and digest mismatch; preserve trustworthy metadata and show unavailable/unverified evidence rather than fabricated content. | Storage/API result and fixture digests; browser detail-state capture. | Linux, macOS, Windows; every approved browser/OS combination. |
| FR-504 | Exercise completed+complete, completed+failed, cancelled+complete, partial with zero findings, and failed-without-report. Verify both state axes survive and no incomplete result appears clean. | Serialized round-trip comparison, API responses, and actual-browser state/announcement record. | Linux, macOS, Windows; every approved browser/OS combination. |
| FR-505 | Run existing CLI commands with web disabled, history absent, and history unavailable; default stdout/stderr, JSON purity, and exit codes stay unchanged. A listener startup failure reports safely as `API_LISTENER_UNAVAILABLE` without breaking CLI use. Test explicit history-save behavior only after its CLI decision is approved. | CLI regression output, startup diagnostic, and repository-tree comparison. | Linux, macOS, Windows. |
| STORE-01 | After explicit history capture is approved and selected, force a transaction failure; require a typed persistence failure, no “saved” claim/partial row, and unchanged scan report/outcome. Additive exit code `6` is tested only if separately approved. | Transaction rollback, safe stderr diagnostic, stdout and exit-status record. | Linux, macOS, Windows. |
...
| SEC-07 | Scan/history/API responses and logs do not reveal tokens, credential URLs, full environments, raw source, SQL/stack traces, or sensitive absolute host paths. | Redaction corpus scan plus sanitized response/log evidence. | Linux, macOS, Windows; every approved browser for rendered output. |
| SEC-08 | Corrupt pages/rows and unsupported future schema produce explicit errors; bytes are preserved and no automatic repair/replacement occurs. | Corruption/future-version matrix and pre/post DB digests. | Linux, macOS, Windows. |
