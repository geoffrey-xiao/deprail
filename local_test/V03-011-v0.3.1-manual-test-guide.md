...
Confirm `scan --help` documents `--format`, `--output`, `--quiet`, and `--verbose`; confirm `fix plan --help` documents `--output`.
...
Expected: exit `2`, `CONFIG_INVALID` on stderr, no root help on stdout.
...
- progress appears only on interactive stderr;
...
Check captured files for ANSI escape bytes (`0x1b`), carriage returns, spinner frames, and animation. Expected: stable line-oriented output with no ANSI or carriage-return updates.
...
- quiet suppresses successful summaries and progress;
